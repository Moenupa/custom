...
_sll () {
    ...
    local -a literals=("--help" "-o" "-e" "-c" "-h")
    local -A literal_transitions=()
    literal_transitions[1]="([4]=1 [0]=1 [3]=1 [2]=1 [1]=1)"
    literal_transitions[0]="([4]=1 [0]=1 [3]=1 [2]=1 [1]=1)"
    local -A command_transitions=()
    command_transitions[1]="([0]=1 [1]=1)"
    command_transitions[0]="([0]=1 [1]=1)"
    local -A subword_transitions
    subword_transitions[0]="([0]=1 [1]=1 [2]=1)"
    subword_transitions[1]="([0]=1 [1]=1 [2]=1)"
    ...
    while [[ $word_index -lt $cword ]]; do
        ...
        if [[ -v "subword_transitions[$state]" ]]; then
            ...
            for subword_id in "${!state_transitions[@]}"; do
                if _sll_subword_"${subword_id}" matches "$word"; then
        ...
        if [[ -v "command_transitions[$state]" ]]; then
            ...
            for cmd_id in "${!state_commands[@]}"; do
                readarray -t candidates < <(_sll_cmd_$cmd_id "" "" | while read -r f1 _; do echo "$f1"; done)
    ...
    local -A literal_transitions_level_0=()
    local -A literal_transitions_level_1=([1]="4 0 3 2 1" [0]="4 0 3 2 1")
    local -A commands_level_0=([1]="0 1" [0]="0 1")
    local -A commands_level_1=()
    local -A subword_transitions_level_0=()
    local -A subword_transitions_level_1=([1]="0 1 2" [0]="0 1 2")
    ...
    for (( fallback_level=0; fallback_level <= max_fallback_level; fallback_level++ )) {
        ...
        for subword_id in "${transitions[@]}"; do
            _sll_subword_$subword_id complete "$prefix"
        done
        eval "local commands_name=commands_level_${fallback_level}"
        eval "local -a transitions=(\${$commands_name[$state]})"
        for command_id in "${transitions[@]}"; do
            readarray -t candidates < <(_sll_cmd_$command_id "$prefix" "" | while read -r f1 _; do echo "$f1"; done)
            if [[ ${#candidates[@]} -gt 0 ]]; then
                __complgen_match "$prefix" candidates matches
            fi
        done
